...
// OpenSCAP - **Requires that the `openscap` plugin is installed on the server.**
//
...
func newOpenSCAP(sdkConfig sdkConfiguration) *OpenSCAP {
	return &OpenSCAP{
...
func (s *OpenSCAP) OpenscapReport(ctx context.Context, nodeID string) (*operations.OpenscapReportResponse, error) {
	...
	request := operations.OpenscapReportRequest{
		NodeID: nodeID,
	}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := utils.GenerateURL(ctx, baseURL, "/openscap/report/{nodeId}", request, nil)
	if err != nil {
	...
	res := &operations.OpenscapReportResponse{
